...
public class Driver {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("There is a no such element exception!") ;
		}
		catch (Exception e) {
			out.add(message("{}.getFirst()", "NoSuchElementException", e.getClass().getSimpleName()));
			System.out.println("There is another exception being thrown!") ;
		}
		...
		for (int i = 0; i < 1000; i++) {
			...
			try {
				System.out.println("We are trying to add " + i + " to the end!") ;
				deque.addLast(i);
				System.out.println("We are adding " + i + " to the end of comp") ;
				comp.addLast(i);
				...
				System.out.println("EXCEPTION BEING THROWN DURING ADDLAST!") ;
				break;
			}
		}
		...
		for (int i = 0; i < 1000; i++) {
			...
			try {
				...
				System.out.println("We are trying to remove the value of i!") ;
				deque.removeLast();
				comp.removeLast();
				if (!edge_check(comp, deque)) {
				...
				System.out.println("EXCEPTION BEING THROWN DURING REMOVELAST") ;
				break;
			}
		}
		...
		for (int i = 0; i < 1000; i++) {
			String old = gist_string(deque);
			try {
				System.out.println("We are trying to add " + i + " to the front!") ;
				deque.addFirst(i);
				comp.addFirst(i);
				if (!edge_check(comp, deque)) {
				...
				System.out.println("EXCEPTION THROWN DURING ADDFIRST") ;
				break;
			}
		}
		...
		for (int i = 0; i < 1000; i++) {
			String old = gist_string(deque);
			try {
				System.out.println("We are trying to REMOVE " + i + " FROM THE BEGINNING!") ;
				deque.removeFirst();
				comp.removeFirst();
				if (!edge_check(comp, deque)) {
				...
				System.out.println("EXCEPTION BEING THROWN DURING REMOVEFIRST CALL!") ;
				break;
			}
		}
		...
		System.out.println("***************************************************************************") ;
		System.out.println("***************************************************************************") ;
		System.out.println("This part has the data in the middle of the data array!") ;
		System.out.println("***************************************************************************") ;
		deque = new MyDeque<>();
		comp = new ArrayDeque<>();
		...
		for (int i = 0; i < 500; i++) {
			...
			try {
				...
				out.add(message(old+".removeFirst()", gist_string(comp), e.toString()));
				System.out.println("EXCEPTION THROWN DURING REMOVEFIRST!") ;
				break;
			}
		}
